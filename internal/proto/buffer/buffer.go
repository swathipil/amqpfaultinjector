...
// Next returns <= n bytes, depending on how much is in the buffer.
...
// readCheck returns _true_ if the buffer is smaller than 'n'.
...
func (b *Buffer) PeekBytes(count int) ([]byte, error) {
	if b.readCheck(int64(count)) {
		return nil, io.EOF
	}

	return b.b[b.i : b.i+count], nil
}
