...
// Text is like String(), but it allows you to control the interpretation of the typecode if you're at the top
// level of a message, where the type codes for certain section types overlap with primitives.
func (at AMQPType) Text(topLevel bool) string {
	// if you're parsing at the top level of a message some codes "overlap" with primitive types
	if topLevel {
		// these are the individual sections available in a top level message
		// they overlap with primitives, so we want to try these first.
		switch at {
		...
		case TypeCodeSASLResponse:
			return "SASLResponse"
		default:
			utils.Panicf("Unhandled top level type code: %d", at)
		}
	}
	...
	case TypeCodeVbin8:
		return "Vbin8"
	default:
		panic(fmt.Sprintf("unexpected encoding.AMQPType: %#v", at))
	}
}
...
func (a Annotations) MarshalJSON() ([]byte, error) {
	// this is a bit gross, but we'll one-way stringify things so it's cosmetically viewable
	// deserializing this is lossy but, at the moment, that's not something we support.
	newM := map[string]any{}

	for k, v := range a {
		switch newK := k.(type) {
		case int64:
			newM[strconv.FormatInt(newK, 10)] = v
		case int:
			newM[strconv.FormatInt(int64(newK), 10)] = v
		case string:
			newM[newK] = v
		default:
			panic(fmt.Errorf("We got a type %T but can't stringify it for annotations", k))
		}
	}
