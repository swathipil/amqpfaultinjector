...
func UnmarshalComposite(r *buffer.Buffer, type_ AMQPType, fields ...UnmarshalField) error {
	...
	for i, field := range fields[:numFields] {
		...
		if err != nil {
			return fmt.Errorf("unmarshaling field %d: %w", i, err)
		}
	}
...
func readListHeader(r *buffer.Buffer) (length int64, _ error) {
	...
	case TypeCodeList8:
		buf, ok := r.Next(2)
		if !ok {
			return 0, errors.New("invalid length for list8")
		}
		_ = buf[1]
	...
	case TypeCodeList32:
		buf, ok := r.Next(8)
		if !ok {
			return 0, errors.New("invalid length for list32")
		}
		_ = buf[7]
...
func readArrayHeader(r *buffer.Buffer) (length int64, _ error) {
	...
	case TypeCodeArray8:
		buf, ok := r.Next(2)
		if !ok {
			return 0, errors.New("invalid length for array8")
		}
		_ = buf[1]
	...
	case TypeCodeArray32:
		buf, ok := r.Next(8)
		if !ok {
			return 0, errors.New("invalid length for array32")
		}
		_ = buf[7]
...
func ReadString(r *buffer.Buffer) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("invalid length for string, with typecode %d", type_)
	}
...
func readBinary(r *buffer.Buffer) ([]byte, error) {
	...
	if !ok {
		return nil, fmt.Errorf("insufficient buffer for length %d for reading binary, only have %d", length, r.Len())
	}
...
func ReadAny(r *buffer.Buffer) (any, error) {
	...
	if err != nil {
		return nil, errors.New("invalid length reading 'any'")
	}
	...
	case TypeCodeDecimal32:
		return readRawType(r, TypeCodeDecimal32, 4)
	case TypeCodeDecimal64:
		return readRawType(r, TypeCodeDecimal64, 8)
	case TypeCodeDecimal128:
		return readRawType(r, TypeCodeDecimal128, 16)
	case TypeCodeChar:
...
type AMQPRawType struct {
	TypeCode AMQPType
	Value    []byte
}

func readRawType(r *buffer.Buffer, expectedTypeCode AMQPType, numBytes int) (*AMQPRawType, error) {
	actualType, err := readType(r)
	if err != nil {
		return nil, err
	}

	if actualType != expectedTypeCode {
		return nil, fmt.Errorf("type code %#00x is not %#00x, can't read raw type", actualType, expectedTypeCode)
	}
...
func readAnyArray(r *buffer.Buffer) (any, error) {
	...
	buf := r.Bytes()
	if len(buf) < 1 {
		return nil, errors.New("invalid length reading 'any' array")
	}
...
func readTimestamp(r *buffer.Buffer) (time.Time, error) {
	...
	n, err := r.ReadUint64()
	ms := int64(n)
	return time.UnixMilli(ms).UTC(), err
}
...
func readUUID(r *buffer.Buffer) (UUID, error) {
	...
	buf, ok := r.Next(16)
	if !ok {
		return uuid, errors.New("invalid length for UUID")
	}
	copy(uuid[:], buf)
...
func readMapHeader(r *buffer.Buffer) (count uint32, _ error) {
	...
	case TypeCodeMap8:
		buf, ok := r.Next(2)
		if !ok {
			return 0, errors.New("invalid length for map8")
		}
		_ = buf[1]
	...
	case TypeCodeMap32:
		buf, ok := r.Next(8)
		if !ok {
			return 0, errors.New("invalid length for map32")
		}
		_ = buf[7]
	...
	if int(count) > r.Len() {
		return 0, errors.New("invalid length for map")
	}
	return count, nil
}
